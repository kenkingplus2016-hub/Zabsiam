...
if (topSplit !== -1 && bottomSplit !== -1) {
    let top = html.substring(0, topSplit);
    let bottom = html.substring(bottomSplit);

    top = top.replace(/<title>Canap&eacute; Menu - Khrua Thai London<\/title>/g, '<title>Party Trays & Delivery - Khrua Thai London</title>');
    top = top.replace(/Canap&eacute; Menu/g, 'Party Trays & Delivery');
    top = top.replace(/<p style="color: #ccc; max-width: 800px; margin: 0 auto; font-size: 1\.1rem; line-height: 1\.6;">[\s\S]*?<\/p>/, '<p style="color: #ccc; max-width: 800px; margin: 0 auto; font-size: 1.1rem; line-height: 1.6;">Authentic Thai party trays and large portion delivery. Perfect for house parties, office lunches, or family gatherings.</p>');
    ...
    console.log('Successfully wrote delivery.html');
} else {
