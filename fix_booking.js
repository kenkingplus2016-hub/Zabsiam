...
// 1. Replace the Menu Selection UI
const uiStartStr = '<!-- Menu Selection Section';
const uiEndStr = '<!-- Allergy Section -->';

const uiStartIndex = content.indexOf(uiStartStr);
const uiEndIndex = content.indexOf(uiEndStr);

if (uiStartIndex !== -1 && uiEndIndex !== -1) {
    const newUI = `<!-- Menu Selection Section (Cart) -->
            <div id="cart-summary-container" style="background:#fff; border:1.5px solid var(--gold); border-radius:12px; padding:20px; margin-bottom:20px; margin-top:20px;">
    ...
    console.log("UI Section replaced.");
} else {
    console.error("Could not find UI markers.");
}

// 2. Replace the main Script
const scriptStartStr = '<script>\n    let selectedPaymentMethod = \'Transfer\';'; // Let's try to match from line 277 <script>
const scriptEndStr = 'loadAllMenus();\n</script>'; // End of the main script block

let scriptStartMatch = content.indexOf('<script>\n    let selectedPaymentMethod');
if (scriptStartMatch === -1) {
    // try finding just <script> that is not leaflet or GTM or tawk
    const scriptTags = content.match(/<script>(.*?)<\/script>/gs);
    if (scriptTags) {
        // find the one containing currentLang
        for (const tag of scriptTags) {
            if (tag.includes('let currentLang')) {
                const sIdx = content.indexOf(tag);
                if (sIdx !== -1) {
                    scriptStartMatch = sIdx;
                    break;
                }
            }
        }
...
if (scriptStartMatch !== -1) {
    ...
    const endScriptTag = content.indexOf('</script>', scriptStartMatch);
    if (endScriptTag !== -1) {
        const newJS = `<script>
    let currentLang = 'th';
    ...
    function changeLang(lang, btn) {
        ...
        document.getElementById('t-connect').innerText = t.connect;
        document.getElementById('t-booking').innerText = t.booking;
        ...
        document.getElementById('l-venue').innerText = t.venue;
        document.getElementById('l-address').innerText = t.address;
        document.getElementById('t-payment').innerText = t.payment;
        document.getElementById('btn-submit').innerText = t.submit;
        document.getElementById('d-title').innerText = t.d_title;
        document.getElementById('d-desc').innerText = t.d_desc;
        document.getElementById('l-allergy').innerText = t.allergy;
        document.getElementById('opt-peanuts').innerText = t.peanuts;
        document.getElementById('opt-milk').innerText = t.milk;
        document.getElementById('opt-seafood').innerText = t.seafood;
        document.getElementById('opt-eggs').innerText = t.eggs;
        if(document.getElementById('opt-msg')) document.getElementById('opt-msg').innerText = t.msg;
        if(document.getElementById('opt-fishsauce')) document.getElementById('opt-fishsauce').innerText = t.fishsauce;
        if(document.getElementById('opt-knorr')) document.getElementById('opt-knorr').innerText = t.knorr;
        document.getElementById('allergy-detail').placeholder = t.other;
    ...
    document.addEventListener("DOMContentLoaded", () => { renderCartSummary(); });
        ...
        content = content.substring(0, scriptStartMatch) + newJS + content.substring(endScriptTag + 9);
        console.log("JS Section replaced.");
    } else {
