...
const gridStartPattern = '<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 30px; text-align: left;">';
const gridStartIndex = html.indexOf(gridStartPattern);
const gridEndIndex = html.indexOf('</div>\n        </div>', gridStartIndex);

if (gridStartIndex !== -1 && gridEndIndex !== -1) {
    let top = html.substring(0, gridStartIndex);
    let bottom = html.substring(gridEndIndex);
    
    // Construct the new items in the desired order
    const newGrid = `${gridStartPattern}
            <!-- Fish 1 -->
...
`;
    ...
    fs.writeFileSync('public/delivery.html', top + newGrid + bottom, 'utf8');
    console.log('Successfully reordered delivery.html grid');
} else {
