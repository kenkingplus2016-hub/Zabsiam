...
const lines = fs.readFileSync('C:/Users/KENDEE/.gemini/antigravity/brain/9d10d3e1-29fa-4160-9fdc-24b2e2180579/.system_generated/logs/transcript_full.jsonl', 'utf8').split('\n');
...
for(let i=0; i<lines.length; i++) {
    if(lines[i].includes('write_to_file') && lines[i].includes('buffet_menu.json')) {
        try {
            ...
            if(obj.tool_calls) {
                obj.tool_calls.forEach(call => {
                    if (call.name === 'write_to_file' && call.args && call.args.CodeContent) {
                        ...
                        if (code.includes('JSON.parse(fs.readFileSync') && code.includes('forEach(category')) {
                            if (!seenScripts.has(code)) {
                                seenScripts.add(code);
                                // Extract the logic inside the script
                                // We replace readFileSync and writeFileSync
                                let logic = code.replace(//g, '')
                                                .replace(//g, '')
                                                .replace(//g, '')
                                                .replace(//g, '')
                                                .replace(//g, '')
...
console.log('Saved to master_recovery.js');
