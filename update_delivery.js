...
html = html.replace(/Canap&eacute; Menu/g, 'Party Trays & Delivery');
html = html.replace(/Explore our exquisite bite-sized Thai canap&eacute;s, perfectly portioned for networking events, cocktail receptions, and elegant gatherings\./g, 'Authentic Thai party trays and large portion delivery. Perfect for house parties, office lunches, or family gatherings.');

// Find the grid container
const startIndex = html.indexOf('<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 30px;">');
const endIndexStr = '<!-- Zab Siam Signature -->';
const endIndex = html.indexOf(endIndexStr);

if (startIndex !== -1 && endIndex !== -1) {
    const endOfSection = html.indexOf('</section>', endIndex);
    const beforeGrid = html.substring(0, startIndex);
    const afterSection = html.substring(endOfSection + 10);
...
`;
    ...
    html = beforeGrid + newGrid + afterSection;
    fs.writeFileSync('public/delivery.html', html, 'utf8');
    console.log('Successfully updated delivery.html');
} else {
