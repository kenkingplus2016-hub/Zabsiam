...
let canapesHtml = fs.readFileSync('public/canapes.html', 'utf8');

// Get everything before the grid
const gridStart = canapesHtml.indexOf('<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 30px;">');
let headerHtml = canapesHtml.substring(0, gridStart);

headerHtml = headerHtml.replace(/<title>Canap&eacute; Menu - Khrua Thai London<\/title>/g, '<title>Party Trays & Delivery - Khrua Thai London</title>');
headerHtml = headerHtml.replace(/Canap&eacute; Menu/g, 'Party Trays & Delivery');
...
// Get everything after the section
const sectionEnd = canapesHtml.indexOf('</section>');
let footerHtml = canapesHtml.substring(sectionEnd);
...
let finalHtml = headerHtml + newGrid + footerHtml;

fs.writeFileSync('public/delivery.html', finalHtml, 'utf8');
console.log('Successfully recreated delivery.html completely clear of Canapes.');
